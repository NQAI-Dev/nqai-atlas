...
MCP публикует все записи как `atlas://records` и шаблон ресурса `atlas://entity/{entity}` для чтения текущего контекста конкретной сущности. Идентификатор сущности в URI должен быть URL-кодирован, например `project%3Anqai-atlas`.
